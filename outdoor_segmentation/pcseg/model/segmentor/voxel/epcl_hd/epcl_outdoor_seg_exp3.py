...
class HD_model():
    def __init__(self, classes = 20, d = 1000, num_features=153, lr = 0.01, **kwargs):
        self.d = d
        self.div = kwargs['div']
...
class EPCLOutdoorSegHD(BaseSegmentor):    
    ...
    def forward(self, batch_dict, return_logit=False, return_tta=False, train_hd=False):
        ...
        if train_hd:
            ...
            self.hd_model.train(z3.F, batch_dict['targets'].feats)
            ...
            point_predict = []
            point_labels = []
            hv, sim, pred_label = self.hd_model.forward(z3.F)
            ...
            return {'point_predict': point_predict, 'point_labels': point_labels, 'name': batch_dict['name'], 'output':z3} #'output_CLIP': output_clip, 'concat_features':concat_feat
