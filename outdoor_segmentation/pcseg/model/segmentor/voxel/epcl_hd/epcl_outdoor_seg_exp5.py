...
class HD_model():
    def __init__(self, classes = 20, d = 1500, num_features=(409, 204, 153), lr = 0.01, **kwargs):
        self.d = d
        self.div = kwargs['div']
    ...
    def train(self, input_points, classification, **kwargs):
        ...
        for i, idx in enumerate(torch.arange(input_points.shape[0]).chunk(self.div)):
            ...
            class_batch[idx_non_zero] = torch.ones((class_batch.shape[0]), dtype=torch.int64, device=self.device)
...
class EPCLOutdoorSegHD(BaseSegmentor):    
    def __init__(
        ...
        self.hd_model = HD_model(device=self.device, div=3, lr=lr, classes=num_class)
