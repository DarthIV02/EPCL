...
class EPCLOutdoorSegHD(BaseSegmentor):    
    ...
    def forward(self, batch_dict, return_logit=False, return_tta=False, train_hd=False):
        ...
        if train_hd:
            ...
            self.hd_model.train(z2.F, batch_dict['targets'].feats)
            ...
            point_predict = []
            point_labels = []
            hv, sim, pred_label = self.hd_model.forward(z2.F)
            ...
            return {'point_predict': point_predict, 'point_labels': point_labels, 'name': batch_dict['name'], 'output':z2} #'output_CLIP': output_clip, 'concat_features':concat_feat
